...
class CompositeIdentityMap extends AbstractIdentityMap
{
    /**
    ...
    protected function getSerialArray(mixed $primaryValue): array
    {
        ...
        foreach ($this->table::PRIMARY_KEY as $column) {
            $serial[$column] = $primaryValue[$column] ?? null;
    ...
    protected function getSerialArrayFromRow(AbstractRow $row): array
    {
        ...
        foreach ($this->table::PRIMARY_KEY as $column) {
            $serial[$column] = $row->get($column) ?? null;
